...
<html>
    ...
    <body>
        ...
        <div id="map" style="height: 690px;  border: 3px solid #AAA;"></div>  
       ...
       <script> //แทรกคำสั่ง JavaScript ไว้ภายใน BODY
        ...
        function popUp(f,l){
           ...
           //คำสั่งเรียกไฟล์ GeoJSON มาแสดง
            var jsonTest = new L.GeoJSON.AJAX(["data/thailand_province.geojson"],{onEachFeature:popUp,style:style, onEachFeature:onEachFeature});

           //JSON Ajax SQL
            var jsonProv = L.geoJSON(null, {onEachFeature:popUp,style:style,onEachFeature:onEachFeature });
                $.ajax({
                dataType: "json",
                url: "json_sql.php",
             ...
             function getColor(area) {
                return area > 12699300000 ? '#F51D07 ':
                area>9880600000 ? '#E74C3C':
                area>6864500000 ? '#7D3C98':
                area>5634800000 ? '#A569BD':
                area>4491200000 ? '#D2B4DE':
                area>3488100000 ? '#AED6F1':
                area>197600000 ? '#ABEBC6':
                '#F9E79F';
                }
            // ใส่สีขอบ
            ...
            legend.onAdd = function(mymap){
                ...
                grades=[197600000,3488100000,4491200000,5634800000,6864500000,9880600000,12699300000],
                labels=[];
            ...
            //คำสั่งสำหรับ query map จาก bootstrap
            var province_sql=
                        <?php
            ...
            var geojson=L.geoJson(province_sql,{
                ...
                mymap.fitBounds(geojson.getBounds()); //คำสั่งให้ซูมเซนเตอร์ไปที่ไฟล์ json
    ...
    mymap.doubleClickZoom.disable(); 
            ...
            var overlays={
				...
				"ขอบเขตประเทศไทย SQL JSON":jsonProv,
                "ขอบเขตประเทศไทย WMS":thailand_bound
